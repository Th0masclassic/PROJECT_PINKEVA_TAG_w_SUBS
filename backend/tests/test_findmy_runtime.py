...
@pytest.mark.asyncio
async def test_api_startup_and_shutdown_remain_usable_while_auth_waits(
    ...
    def slow_login(*args, **kwargs):
        ...
        return AppleSession("test", "test-token")
    ...
    monkeypatch.setattr(main, "PremiumRetentionWorker", Worker)
    ...
    async def exercise():
        async with main.lifespan(app):
            ...
            assert app.state.findmy_auth.status()["phase"] == "authenticating"
            assert not app.state.findmy_auth.status()["session_available"]
            release.set()
