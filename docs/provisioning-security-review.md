...
**Review scope:** protocol-v1.8 QR-free dual-network authorization, identity generation, claim/resume, authenticated mobile discovery/setup, App-to-Tag BLE transfer, ESP32 storage, dual advertisement scheduling, public DULT sound, one-owner enforcement, authenticated release, account-level cloud subscription boundaries, and subscription cancellation.
...
- The **Apple advertisement key** is the 28-byte X coordinate defined by the existing experimental finder protocol. The Apple private scalar remains encrypted in the backend.
- The **Google identity key (EIK)** is a random 32-byte backend secret encrypted with session-bound AES-256-GCM. The current 20-byte secp160r1 counter-zero development EID is derived from it and sent to the tag; the EIK is not.
- Each **advertisement-key identifier** is SHA-256 over the public bytes. Fingerprints are safe for matching/idempotency but must not substitute for authorization.
...
The advertisement values are public material, but an attacker who replaces either one or the selector can deny service or bind a tag to the wrong account. The flow therefore protects integrity and ownership even though confidentiality of those public values is not the main security goal.
...
| Existing/possibly written identity | The same allocation is resumed. Unknown/mismatched fingerprints or selector enter recovery; no timeout path generates replacement material. |
| Backend/database disclosure | Apple private scalar and Google EIK are AES-GCM ciphertext; the envelope key is external and user-facing database roles have no table access. |
| BLE passive eavesdropping or unauthorized setup | **Open in the checked-in development no-bond/no-bootstrap profile.** Authentication and key confidentiality require a reviewed application-layer secure channel plus restored per-device bootstrap verification before production. |
...
| Short ATT MTU / fragmented write | Handle-bound prepared staging enforces exact lengths, including 28-byte Apple, 20-byte Google, one-byte selector, 32-byte proof/control, and 64-byte reset; partial data is zeroed and never committed. |
...
| Owner removes/transfers tracker | Two-phase release verifies both fingerprints, selector, and tag-control HMAC; erases both identities/selector/control; ends one ownership row; revokes the allocation; and permits a new bundle only after complete empty confirmation. |
| Subscription during removal | Tracker removal does not cancel billing. The account subscription remains active for the owner's other and future trackers; only tracker ownership and finder credentials are revoked. |
| Reboot after identity receipt | A complete dual-network bundle immediately restores the alternating Apple and Google advertisements. |
| Subscription expires | Premium APIs stop server-side for the account; both finder advertisements continue unchanged and no BLE synchronization is requested. |
...
3. **Cloud subscription enforcement must remain server-side.** Keep tests proving that subscription expiry pauses premium cloud features for every tracker on the account without stopping either finder advertisement. Do not reintroduce billing packets, expiry clocks, or renewal BLE dependencies.
...
5. **Hardware testing is partial.** The protocol-v1.9/firmware-0.6.0 classic-ESP32 image builds cleanly, including the dual-slot scheduler, owner ring authorization, and DULT buzzer driver, but full Apple/Google frame capture, CPT-9019A-SMT-TR electrical validation, sound-command capture, complete claim, partial-write recovery, power loss, reset verification, reboot, and notification ordering still need testing on the exact target board and both mobile platforms.
...
7. **Finder-network/legal requirements remain external.** Experimental interoperability is not commercial authorization. Google's published specification says a switchable accessory must use only one finding network at a time, while the current firmware intentionally time-slices two; Apple owner Find My behavior requires MFi. Written platform approval, official enrollment, certified rotation and owner ring/play-sound behavior, complete DULT and anti-stalking behavior, RF/power testing, certification, privacy review, retention/deletion, and regulatory work remain release blockers.
8. **Payment-provider cancellation needs a worker.** Account revocation, invalid post-Checkout account state, or migration consolidation can end a local billing row and atomically enqueue its provider subscription ID. A normal tag release does not cancel account billing. Provider billing is not guaranteed stopped until a retrying worker succeeds and a signed webhook confirms the terminal state.
...
- Normal transfer is two-phase and rotates identities: authenticated reset erases both finder identities, selector, and control key; backend completion ends ownership and revokes the allocation without changing account billing; and the next owner receives a new bundle.
