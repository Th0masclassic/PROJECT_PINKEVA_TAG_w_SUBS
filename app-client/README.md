...
`TagProvisioner` is the protocol-v1.8 React Native service layer. The UI supplies the selected BLE peripheral, a Supabase access-token callback, a stable idempotency key, and the platform setup preference. No QR scan or user-entered setup secret is required.
...
6. Wait for `Ready / Success`, then read and compare both 32-byte fingerprints and the startup preference after NVS read-back. Firmware 0.5.0 advertises both identities regardless of that preference.
